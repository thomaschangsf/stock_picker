...
**Locked — Phase 2a scope (all three specialist tracks):** Implement **Scout (technical / market history)**, **Auditor (fundamental / SEC–family tools behind Squid)**, and **Analyst (sentiment / news)** together for Phase 2a, each emitting **Pydantic-validated** JSON into the existing LangGraph edges (Manager unchanged until a follow-on milestone unless explicitly extended). **Tradeoff accepted:** broader scope, more integrations, kill-criteria, and operational tuning than a single-track Phase 2a.

**Locked — Phase 2a MCP transport:** **Option B — MCP over HTTP/SSE** from each MCP server, reachable on **`mcp_internal`** only (no “MCP on stdio per spawn” for Phase 2a; no “host-only direct vendor HTTP” as the primary architecture). **Rationale:** better fit for **continuous / high-volume** streaming than stdio subprocess churn, and clearer **network segmentation** when listeners are confined to the internal Docker network (plus existing Squid path for fundamental egress).

**Locked — Phase 2a orchestrator placement (host dev + tight bind):** **Option 3 — hybrid.** The **LangGraph orchestrator runs on the host** (e.g. `uv run stock-picker …`) for iteration speed; MCP HTTP/SSE services run in **Docker on `mcp_internal`**. Any `ports:` published for host access MUST be **`127.0.0.1:<hostPort>:<containerPort>`** (never bare `0.0.0.0`). **Phase 2a MUST** add **unauthenticated-local-process protection** via an **MCP HTTP bearer token** (or equivalent) shared via env / local secret file—**not** committed to git. **Later profile:** optionally move the orchestrator into compose (**Option 2**) for a “prod-like” all-internal deployment once MCP servers stabilize.

**Locked — Phase 2a Scout market history source:** **Default to on-disk Parquet** under `datasets/market_data/{SYMBOL}.parquet` (append/merge policy as in `specs/backtesting-v1.md`). **Also allow live fetch** exclusively via the **Scout MCP** (HTTP/SSE), using **documented HTTP APIs** (e.g. **Alpha Vantage optional**, API key + rate limits; additional providers only via small PRs + allowlist as for other egress). The graph host does **not** call vendors directly; it reads Parquet or calls Scout MCP, which performs fetches and returns **Pydantic-shaped** payloads including **provenance** (`source`, `as_of`, `provider`).

**Locked — Phase 2a Auditor / fundamental egress (Squid):** **SEC family allowlist** maintained **in-repo** in `infra/phase1/squid-fundamental/squid.conf` (and/or a small include file), extended **only** when a tool proves a **new SEC/EDGAR-related hostname** is required. **Policy:** prefer **documented additions** in PRs (hostname + justification + tool reference); avoid “wide” unrelated domains. **Tradeoff accepted:** slightly broader egress than a single-host allowlist, in exchange for fewer **brittle outages** as SEC edges/CDNs shift.

**Locked — Phase 2a Analyst (hybrid: SEC baseline + optional Finnhub):** **Baseline = SEC-only analysis** for Phase 2a: **SEC RSS/Atom** plus structured inputs from **Auditor/Scout** (e.g. filings references, material events) **without** requiring per-ticker issuer IR feed curation. **Optional enhancement:** **Finnhub** (free tier) for a **tiny number of HTTP calls per run**—target **1–2 Finnhub API requests per daily batch** (exact endpoints and caps **documented and enforced** in code + `run_budget`; API key via env only). When Finnhub is **disabled** (default for zero-cost), Analyst still produces a defensible output from **SEC baseline** only. **Squid / allowlist:** extend the **SEC family** rules as today; add **`finnhub.io` (and any documented CDN/subdomains Finnhub requires)** **only** when the Finnhub option is enabled, via the same **PR + justification** discipline as other non-SEC domains. **Documentation:** what is in-scope vs out-of-scope for Analyst, plus **future TODOs**, lives in **[docs/analyst-coverage.md](../docs/analyst-coverage.md)** (kept current as scope evolves).

**Superseded (do not implement):** the earlier **“Pair 2” issuer IR RSS** baseline—dropped as **unmaintainable at S&P scale**; issuer IR may return later as an **optional watchlist** (see TODOs in `docs/analyst-coverage.md`).
